...
import { mergePdfFiles } from '../../utils/fileTools/pdfUtils';
...
export const MergePdf = () => {
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      const merged = await mergePdfFiles(files, (pct) => setProgress(pct));
      setResult(merged);
    } catch (err) {
  ...
  const handleDownload = () => {
    if (result?.blob) {
      downloadBlob(result.blob, result.name || 'merged-document.pdf');
    }
  };
  ...
  return (
    <ToolLayout tool={tool}>
      {result ? (
        ...
        <ProcessingStatus
          progress={progress}
          title="Merging your PDF documents..."
          subtitle="Combining all pages in your chosen sequence directly inside your browser."
        />
      ) : (
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
                <p className="text-xs text-slate-500">
                  Tip: Use the arrow buttons to place your PDF files in the exact order you want them merged.
                </p>
                <Button
                  type="button"
                  variant="primary"
